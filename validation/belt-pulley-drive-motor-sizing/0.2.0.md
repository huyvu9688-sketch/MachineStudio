...
- Validation date: `2026-08-18` (Stage 4); release date: `2026-08-18`
  (Stage 6)
...
disclosed in `0.1.0`'s own record). `acceleration_torque` and
`inertia_ratio` are likewise exercised end to end by the reference-example
test (the compute path runs without error and produces a positive,
correctly-ordered result) but are **not** asserted against AutomationDirect's
own printed `T_accel=0.46 lb-in` (recomputed `0.4649`) or `inertia
ratio=9.6` figures — carried over unchanged from `0.1.0`'s own disclosed
gap: both figures depend on the candidate motor's own rotor inertia, which
the source's own printed worked example never states as an independent
figure, and two different ways of back-solving it from the source's own
two printed downstream figures disagree by roughly 15-20%.
...
against a printed figure — none exists) — an equality check, not
independent evidence, so `deceleration_torque` sits on top of the same
rotor-inertia gap just disclosed above rather than escaping it.
`effective_torque` computes to a positive, finite value bounded between
`load_torque` and `momentary_torque` but is **not** claimed to match any
printed figure — the source gives none (see "Unsupported Conditions").

## Independent Method or Tool Comparison

**Satisfied (2026-08-18).** Two independent benchmarks:

1. Carried over from `0.1.0`: the force/load-torque single-expression
   reimplementation, a 2,187-scenario (`3^7`) property sweep, algebraic
...
disagreement between Oriental Motor and AutomationDirect, the confirmed
arithmetic slip in AutomationDirect's own friction-force line, and the
rotor-inertia back-solving disagreement disclosed under "Reference
Examples" above (two ways of back-solving the candidate motor's own rotor
inertia from the source's own printed figures disagree by roughly
15-20%, so `acceleration_torque`/`deceleration_torque`/`inertia_ratio`
are not asserted against the source's own printed values) — see
`validation/belt-pulley-drive-motor-sizing/0.1.0.md` for the full account.
...
- `acceleration_torque`, `deceleration_torque`, and `inertia_ratio` are
  not validated against AutomationDirect's own printed figures
  (`T_accel=0.46 lb-in`, recomputed `0.4649`; inertia ratio `9.6`) —
  carried over unchanged from `0.1.0`'s own disclosed gap: both source
  figures depend on the candidate motor's own rotor inertia, which the
  source's own worked example never states independently, and two ways of
  back-solving it from the source's own two printed downstream figures
  disagree by roughly 15-20%. `deceleration_torque` is checked only for
  equality against `acceleration_torque` (an internal-consistency check,
  not independent evidence), so it inherits this same gap rather than
  escaping it.
...
`reviewDate` as `"2026-08-14"` — an early-scaffolding-stage value from the
original plan text, predating the Task 5-onward work (independent
benchmarks, reference example, conformance sweep) that this record's own
`2026-08-18` dates actually reflect. Left unamended: `validation.ts` is
part of the sealed, released `0.2.0` package
(`ModuleManifest.contentHash`), and this project's released-module-version
immutability invariant (`CLAUDE.md`) applies to it the same as any other
...
2026-08-18 — coexisting with, not superseding, `0.1.0`.
