 ...
 * — a checkbox is always definitively true or false, never empty. A
 * `"default"`-sourced field is never already complete: `ResolvedInputSource`
 * (`lib/db`) has no `value` on its `"default"` member at all — this
 * codebase has no built-in-default mechanism yet, so `source === "default"`
 * always means "nothing set", never "pre-filled from a registry constant".
 * Any other source (`"manual"` | `"workflow"`) already carries a real value.
 */
function isFieldInitiallyComplete(field: ModuleInputFieldView): boolean {
  ...
  return field.resolved.source === "manual" || field.resolved.source === "workflow";
}
 ...
 * "Generic Module Workspace"). `source === "default"` only means "nothing
 * was manually entered, linked, or workflow-supplied" — `ResolvedInputSource`
 * (`lib/db`) carries no `value` at all on its `"default"` member, so this
 * codebase has no built-in-default mechanism yet: a `"default"` field is
 * always genuinely empty. When such a field is also `required`, this renders
 * "Not set" in the error color instead of "Default", so an empty required
 * field never looks harmlessly pre-filled.
 */
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <SourceBadge source={field.resolved.source} required={field.required} />
        {field.loadCase !== null ? (
