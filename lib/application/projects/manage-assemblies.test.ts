...
describe.skipIf(!liveDatabaseAvailable)(
  "manage-assemblies (live database)",
  () => {
    ...
    beforeAll(async () => {
      ({ createMachineAssembly, renameMachineAssembly } =
        await import("./manage-assemblies"));
    ...
    it("creates a nested assembly under a real parent in the same configuration", async () => {
      const { ownerId, configurationId } = await fixture();
      const root = await createMachineAssembly(
        { configurationId, name: "X axis" },
        ownerId,
      );
    ...
    it("rejects a parent assembly from a different configuration", async () => {
      ...
      const result = await createMachineAssembly(
        {
          configurationId,
          parentId: foreignParent.assembly.id,
          name: "Should fail",
        },
        ownerId,
      );
    ...
    it("renames an assembly owned by the caller", async () => {
      const { ownerId, configurationId } = await fixture();
      const created = await createMachineAssembly(
        { configurationId, name: "Original" },
        ownerId,
      );
      ...
      expect(result.ok).toBe(true);
      const tree = await projects.loadConfigurationTree(
        configurationId,
        ownerId,
      );
