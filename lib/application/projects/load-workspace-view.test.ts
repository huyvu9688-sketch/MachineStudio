...
describe.skipIf(!liveDatabaseAvailable)("loadWorkspaceView (live database)", () => {
  ...
  it("returns the project list and a full tree for the default (most recent) project", async () => {
    ...
    const assembly = await repo.createAssembly({ configurationId: config.id, name: "X axis" });
    ...
    expect(view.projects.map((p) => p.id).sort()).toEqual([older.id, newer.id].sort());
    ...
    expect(view.selectedProject?.configurations[0].assemblies).toHaveLength(1);
    expect(view.selectedProject?.configurations[0].assemblies[0].moduleInstances).toHaveLength(1);
  });
