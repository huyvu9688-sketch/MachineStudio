...
describe.skipIf(!liveDatabaseAvailable)("manage-design-assumptions (live database)", () => {
  ...
  it("creates an assembly-scoped design assumption in the same configuration", async () => {
    ...
    const result = await createMachineDesignAssumption(
      { configurationId, assemblyId: assembly.id, statement: "Local assumption." },
      ownerId,
    );
  ...
  it("rejects an assembly from a different configuration", async () => {
    ...
    const result = await createMachineDesignAssumption(
      { configurationId, assemblyId: foreignAssembly.id, statement: "Should fail." },
      ownerId,
    );
