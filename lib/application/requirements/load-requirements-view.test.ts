...
describe.skipIf(!liveDatabaseAvailable)(
  "load-requirements-view (live database)",
  () => {
    ...
    it("returns null for an unknown or unowned configuration", async () => {
      ...
      expect(
        await loadRequirementsView(configurationId, strangerId),
      ).toBeNull();
      expect(
    ...
    it("reports a requirement's verification status by whether acceptance criteria are recorded", async () => {
      ...
      if (!withCriteria.ok || !withoutCriteria.ok)
        throw new Error("fixture setup failed");
      await createRequirementAcceptanceCriterion(
        {
          requirementId: withCriteria.requirement.id,
