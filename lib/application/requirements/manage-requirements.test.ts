...
describe.skipIf(!liveDatabaseAvailable)("manage-requirements (live database)", () => {
  ...
  it("creates a machine-level requirement", async () => {
    ...
    const result = await createMachineRequirement(
      { configurationId, code: "REQ-01", statement: "Positions within 0.1 mm." },
      ownerId,
    );
  ...
  it("creates an assembly-scoped requirement in the same configuration", async () => {
    const { ownerId, configurationId } = await fixture();
    const assembly = await projects.createAssembly({ configurationId, name: "X axis" });

    const result = await createMachineRequirement(
      { configurationId, assemblyId: assembly.id, code: "REQ-02", statement: "Holds under load." },
      ownerId,
    );
  ...
  it("rejects an assembly from a different configuration", async () => {
    ...
    const result = await createMachineRequirement(
      { configurationId, assemblyId: foreignAssembly.id, code: "REQ-03", statement: "Should fail." },
      ownerId,
    );
  ...
  it("rejects a blank code or statement", async () => {
    ...
    if (!blankStatement.ok) expect(blankStatement.error.code).toBe("invalid_input");
  });
  ...
  it("adds an acceptance criterion to an owned requirement", async () => {
    ...
    const result = await createRequirementAcceptanceCriterion(
      { requirementId: requirement.requirement.id, statement: "Measured within tolerance." },
      ownerId,
    );

    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.acceptanceCriterion.requirementId).toBe(requirement.requirement.id);
  });
