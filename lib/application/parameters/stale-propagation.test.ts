...
describe.skipIf(!generatedClientAvailable)(
  ...
  () => {
    ...
    it("marks a multi-level dependency chain stale", async () => {
      ...
      // Re-execute B and C now that they're linked, for a fresh "current" run
      // each (confirming a link staling the prior run is its own behavior,
      // asserted below).
      const bRun2 = await executeModuleInstance({
        moduleInstanceId: b.moduleInstanceId,
        ownerId: s.ownerId,
      });
      const cRun2 = await executeModuleInstance({
        moduleInstanceId: c.moduleInstanceId,
        ownerId: s.ownerId,
      });
      expect(bRun2.ok && cRun2.ok).toBe(true);
      if (!bRun2.ok || !cRun2.ok) return;
      ...
      expect(await isRunStale(bRun2.run.id)).toBe(true);
      expect(await isRunStale(cRun2.run.id)).toBe(true);
    });
    ...
    it("rolls back the whole transaction when the write fails, including the stale marks", async () => {
      ...
      expect(bRun2.ok).toBe(true);
      if (!bRun2.ok) return;
      expect(await isRunStale(bRun2.run.id)).toBe(false);
