...
describe("generateModuleScaffold", () => {
  it("generates the full placeholder file set under lib/modules/<id>/<version>/", () => {
    ...
    const names = result.files.map((f) => f.path.replace(`${result.moduleDir}/`, ""));
  ...
  it("assembles and seals the package in index.ts with a camelCase export", () => {
    ...
    expect(index?.contents).toContain("export const ballScrewModule: ModulePackage");
  ...
  it("maps placeholder ports to released parameters so a fresh scaffold conforms", () => {
    ...
    expect(manifest?.contents).toContain('asParameterId("motion.axis.payload_mass")');
    expect(manifest?.contents).toContain('asParameterId("motion.axis.thrust_force")');
  });
  ...
  it("imports only the engine public surface in generated sources", () => {
    ...
    for (const file of result.files) {
      ...
      const bareImports = [...file.contents.matchAll(/\bfrom\s+"([^"]+)"/g)].map((m) => m[1]);
  ...
  it("honors a custom version", () => {
    const result = generateModuleScaffold({ moduleId: "ball-screw", version: "1.2.0" });
  ...
  it("rejects a malformed version", () => {
    expect(() => generateModuleScaffold({ moduleId: "ball-screw", version: "1.0" })).toThrow();
  });
});
