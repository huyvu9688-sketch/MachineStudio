...
describe("resolveReflectedLoadInertia / resolveTotalSystemInertia / resolveInertiaRatio", () => {
  it("sums the three reflected terms, then adds the motor rotor and drive-roller inertia", () => {
    ...
    expect(inertiaRatio).toBeCloseTo((6e-3 + 5e-4) / 1e-5, 6);
  });

  it("0.2.1: includes the drive roller's own inertia in the ratio, not just reflected load", () => {
    // A heavy drive roller relative to a light load: the pre-0.2.1 formula
    // (reflectedLoadInertiaKgM2 / motorRotorInertiaKgM2 alone) would report
    // a small, passing ratio while the true load-side/rotor ratio (every
    // non-rotor inertia) is far larger.
    const withoutDriveRoller = 1e-3 / 1e-5;
    const { inertiaRatio } = resolveInertiaRatio({
      reflectedLoadInertiaKgM2: 1e-3,
      driveRollerInertiaKgM2: 1, // a heavy drive roller
      motorRotorInertiaKgM2: 1e-5,
    });
    expect(inertiaRatio).toBeGreaterThan(withoutDriveRoller);
    expect(inertiaRatio).toBeCloseTo((1e-3 + 1) / 1e-5, 6);
  });
});
