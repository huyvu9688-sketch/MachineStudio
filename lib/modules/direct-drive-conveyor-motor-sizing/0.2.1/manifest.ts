...
//
// 0.2.1 (this version): formula-correctness patch, ports and scope
// otherwise unchanged from 0.2.0 -- resolveInertiaRatio (math.ts) now
// includes the drive roller's own inertia in its numerator. 0.2.0 computed
// the ratio from resolveReflectedLoadInertia's own idler+belt+load total
// alone, silently excluding the drive roller inertia that
// resolveTotalSystemInertia (the very next step) already adds to the motor
// rotor -- a heavy/large-diameter drive roller could pass the inertia-ratio
// check while the true load-side/rotor ratio was far above the declared
// maximum. See math.ts's own doc comment and validation.ts "deviations".
