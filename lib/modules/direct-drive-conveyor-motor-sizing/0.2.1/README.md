...
1. **Gravity is no longer an editable input.** `math.ts`'s own
   `resolveFrictionForce` hardcodes `STANDARD_GRAVITY_M_PER_S2 = 9.80665`
...
2. **`inertia_ratio_maximum` now resolves to a founder-directed recommended
   default of 10:1** (`motor_sizing.direct_drive_conveyor.
...
(`lib/modules/direct-drive-conveyor-motor-sizing/0.1.0/`) — an engineer
who wants `0.2.0`'s behavior on an existing instance archives it and adds
a fresh `0.2.0` instance, the same migration story
`belt-pulley-drive-motor-sizing@0.2.0` already established. Full record:
