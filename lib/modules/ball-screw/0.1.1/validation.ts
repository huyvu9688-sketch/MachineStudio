...
export const validation: ValidationRecord = {
  ...
  deviations: [
    ...
    "0.1.1: resolveDriveTorque (math.ts) now takes the magnitude of the combined drive-plus-preload-friction torque term before dividing by gear ratio, rather than letting a signed axialForceN (an assisting/overhauling load, e.g. a vertical-axis case with a counterweight or overhauling load) carry through to a negative screw.drive_torque output. Not a formula source change (still Oriental Motor's T_L = F*P/(2*pi*eta) + mu0*F0*P/(2*pi), divided by i) — required torque is the drive's torque capacity, the same quantity regardless of load direction, and the registry's own screw.drive_torque range (min: 0) already declared this. Every reference example above reproduces the identical figures under 0.1.1 (all use a positive resisting axial load, so the magnitude is a no-op on the published cases); the fix only changes behavior for a signed input, exercised by a new boundary test in math.test.ts. A release audit (2026-08-20) found the engine SDK did not enforce declared parameter ranges at all (fixed separately at the engine level, ENGINE_SDK_VERSION 1.1.0) and that this module's own negative output was the concrete case that motivated closing that gap.",
  ],
};
