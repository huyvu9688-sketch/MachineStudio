...
export interface DriveTorqueInput {
   ...
   * re-deriving gravity/friction. May be signed (e.g. an assisting/overhauling
   * load on a vertical axis produces a negative value) — the sign describes
   * force direction, not required drive capacity (0.1.1 fix: the module now
   * reports required torque as a magnitude — see {@link resolveDriveTorque}).
   */
 ...
 *
 * 0.1.1 fix: takes the magnitude of the combined torque term before dividing
 * by gear ratio. `screw.drive_torque`'s own registry range is `min: 0` (it
 * is the torque *capacity* the drive must supply, the same quantity
 * regardless of whether the axial force resists or assists motion) — 0.1.0
 * let a signed `axialForceN` (a real case: an assisting/overhauling load on
 * a vertical axis) produce a negative `loadTorqueNm` that violated the
 * declared range and silently propagated a wrong-sign value into any
 * downstream safety-factor division (coupling/drive-train's own
 * `drive_torque` input port). See the release audit this closes.
 */
export function resolveDriveTorque(input: DriveTorqueInput): DriveTorqueResult {
  ...
  return {
    loadTorqueNm:
      Math.abs(driveTermNm + preloadFrictionTermNm) / input.gearRatio,
  };
}
