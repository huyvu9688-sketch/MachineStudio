...
export const validation: ValidationRecord = {
  ...
  deviations: [
    ...
    "0.2.1: resolveOperatingSpeed (math.ts) now rejects a motion profile where 2*acceleration_time exceeds index_time, not just acceleration_time >= index_time. Neither primary source's own worked example approaches this boundary (both reference examples above use acceleration_time well under half of index_time), so neither reproduced figure is affected by this fix. The prior check let a symmetric trapezoidal move with an impossible negative implied cruise time (e.g. acceleration_time=0.75s, index_time=1s -- 2*0.75=1.5 > 1) compute a speed and downstream torque figure instead of being rejected, because the trapezoid-area formula's own (t_index - t_A) denominator stays positive even past that boundary. A release audit (2026-08-20) found the gap.",
  ],
};
