...
// otherwise unchanged from 0.2.0 -- resolveOperatingSpeed (math.ts) now
// rejects a motion profile where `2*acceleration_time > index_time`. The
// prior check only required `acceleration_time < index_time`, which is
// sufficient for the trapezoid-area formula's own `(t_index - t_A)`
// denominator to stay positive but not for the move to be physically
// achievable (a symmetric trapezoid needs both ramps, `2*t_A`, plus a
// non-negative cruise segment). See validation.ts "deviations".
