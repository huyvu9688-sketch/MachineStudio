...
export interface OperatingSpeedInput {
  ...
  /** Total move time for one index, in s. Must be >= 2*accelerationTimeS. */
 ...
 *
 * 0.2.1 fix: a symmetric trapezoidal move needs both ramps (accel + decel,
 * each `t_A`) plus a non-negative cruise segment to fit inside `t_index`:
 * `t_index >= 2*t_A`, i.e. `t_cruise = t_index - 2*t_A >= 0`. 0.1.0/0.2.0
 * only checked `t_A < t_index` -- true for the trapezoid-area formula's own
 * `(t_index - t_A)` denominator to stay positive, but not sufficient for
 * the move to be physically achievable: `2*t_A > t_index` describes a
 * cruise time the formula's own algebra never surfaces as negative (the
 * denominator is `t_index - t_A`, not `t_index - 2*t_A`), so an infeasible
 * profile silently computed a speed/torque figure instead of being
 * rejected. See the release audit this closes.
 */
export function resolveOperatingSpeed(
  ...
  if (2 * input.accelerationTimeS > input.indexTimeS) {
    fail(
      "2*accelerationTimeS must not exceed indexTimeS (a symmetric trapezoidal move needs both ramps, plus a non-negative cruise segment, to fit inside indexTimeS).",
    );
  }
  assertPositive("gearRatio", input.gearRatio);
