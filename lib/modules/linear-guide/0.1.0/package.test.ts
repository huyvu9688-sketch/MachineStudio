...
import { linearGuideModule } from "./index";
import {
  asQuantity,
  axisVector,
  orientationValue,
...
// Pinned by `npm run module:source-hash -- linear-guide 0.1.0` — see
...
const EXPECTED_SOURCE_HASH = "e47c2933546e88ed";
...
describe("linear-guide 0.1.0 validation record completeness", () => {
  it("records the finalized solo-review reviewer and review date", () => {
    expect(linearGuideModule.validation.reviewer).toBe(
      "Solo validation — IKO independent-benchmark substitute",
    );
    expect(linearGuideModule.validation.reviewDate).toBe("2026-08-12");
  });
  ...
  it("rejects provisional/draft release language in supported-use limits and deviations", () => {
    ...
    const strings = [
      ...linearGuideModule.validation.supportedUseLimits,
      ...linearGuideModule.validation.deviations,
    ];
