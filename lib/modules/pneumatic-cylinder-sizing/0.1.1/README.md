# pneumatic-cylinder-sizing 0.1.1
...
UI-clarity-only patch on 0.1.0: `ui.ts`'s `incline_angle` field now has
`help` text stating 0deg = horizontal, 90deg = vertical, and that
intermediate values are a real blended orientation (sin/cos split between
gravity and friction), not a horizontal/vertical toggle. No formula, port,
output, or catalog-matching behavior changed from 0.1.0 — see
`validation.ts` "deviations" (no separate `validation/<module>/0.1.1.md`
file, matching `ball-screw@0.1.1`'s own in-code-only precedent for a
non-formula patch).
