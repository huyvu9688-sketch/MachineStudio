...
// module definition of done, item 10). 0.1.1 carries 0.1.0's own
// validated content forward unchanged (see "deviations" below) -- no
// formula, port, or output changed, so no new reference-example or
// independent-benchmark work was needed for this patch.
...
export const validation: ValidationRecord = {
  ...
  deviations: [
    ...
    "0.1.1 adds help text to the incline_angle field (./ui.ts) clarifying that 0 deg = horizontal, 90 deg = vertical, and intermediate values are a real blended orientation, not a two-way toggle -- a UI-clarity fix, not a formula, port, or output change. 0.1.0's own computed results are reproduced unchanged (see math.test.ts, unchanged from 0.1.0).",
  ],
};
