...
/**
 * `T1 = max(|Ta+TL|, |Td-TL|)` -- the governing peak/starting torque across
 * both ramp phases of the cycle.
 *
 * 0.3.1 fix: 0.1.0/0.2.0/0.3.0 reported only the acceleration-phase torque
 * (`Ta+TL`), silently assuming it always governs. `resolveEffectiveTorque`
 * (below) already treats the deceleration phase's own torque as
 * `Td-TL` -- structurally, a fast stop (`decelerationTimeS <<
 * accelerationTimeS`) drives `Td` arbitrarily high while `Ta` stays modest,
 * so the true peak the motor must supply during that phase can exceed the
 * accel-phase figure by an order of magnitude or more, with the SDK's own
 * `required_torque_safety_factor` then applied to the wrong (too-small)
 * base. See the release audit this closes.
 */
export function resolveMomentaryTorque(
  ...
  const accelPhaseTorqueNm = input.accelerationTorqueNm + input.loadTorqueNm;
  const decelPhaseTorqueNm = Math.abs(
    input.decelerationTorqueNm - input.loadTorqueNm,
  );

  return {
    momentaryTorqueNm: Math.max(accelPhaseTorqueNm, decelPhaseTorqueNm),
  };
}
...
/**
 * `Trms = sqrt(((Ta+TL)^2*t1 + TL^2*t2 + (Td-TL)^2*t3 + TL^2*t4) / tf)` --
 * Oriental Motor's own generic per-phase effective-load-torque formula
 * (jp.oriental_motor.motor_sizing_calculations, p. 6), extended with a
 * dwell-phase (`t4`) term.
 *
 * 0.3.1 fix: 0.1.0/0.2.0/0.3.0 treated dwell as contributing zero torque
 * (only counting toward `tf`), which is correct only when nothing needs
 * holding while stopped. This module's own `supportedUseLimits`
 * (validation.ts) already documents "load torque is assumed constant
 * across all four motion phases" -- true for orientation/mass/friction,
 * which do not change mid-cycle -- but the RMS formula did not actually
 * apply that stated assumption to the dwell phase, silently reporting the
 * carriage as torque-free while stopped. For a vertical or inclined axis
 * (`load_torque` includes a gravity-holding component from
 * `resolveDriveForce`), a long dwell then materially undercounts the
 * motor's real thermal/RMS duty. See the release audit this closes.
 */
export function resolveEffectiveTorque(
  ...
  const decelTerm =
    ...
    input.decelerationTimeS;
  const dwellTerm = input.loadTorqueNm ** 2 * input.dwellTimeS;

  return {
    effectiveTorqueNm: Math.sqrt(
      (accelTerm + runTerm + decelTerm + dwellTerm) / input.cycleTimeS,
    ),
  };
}
