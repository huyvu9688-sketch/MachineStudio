...
`required_torque` (0.3.1) is governed by whichever ramp phase demands more
torque -- `max(|T_A+T_L|, |T_D-T_L|)`, not always the acceleration phase;
`effective_torque` (0.3.1: now includes a dwell holding term) is additive,
not a replacement.
