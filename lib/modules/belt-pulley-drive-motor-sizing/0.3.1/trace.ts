...
export function buildTrace(input: TraceInput): CalculationTrace {
  ...
  const requiredTorqueStep = {
    ...
    expression:
      "T1 = max(|T_A+T_L|, |T_D-T_L|); T_req = T1*Sf; P_req = T_req*N_op",
    inputs: [
      {
        label: "Sf",
    ...
    sources: ORIENTAL_MOTOR_SOURCE,
    notes: [
      "T_req (0.3.1) is governed by whichever ramp phase demands more torque, not always the acceleration phase: T1 = max(|T_A+T_L|, |T_D-T_L|) -- a fast deceleration_time relative to acceleration_time can make the deceleration phase govern. Additive to, not replaced by, effective_torque below.",
    ],
  };
  ...
  const effectiveTorqueStep = {
    ...
    expression:
      "Trms = sqrt(((T_A+T_L)^2*t1 + T_L^2*t2 + (T_D-T_L)^2*t3 + T_L^2*t4) / tf)",
    inputs: [],
    outputs: [
      {
        label: "Trms",
    ...
    sources: ORIENTAL_MOTOR_SIZING_CALCULATIONS_SOURCE,
    notes: [
      "Dwell time (t4) now contributes its own T_L^2*t4 holding term (0.3.1 fix) as well as counting toward tf: load_torque is assumed constant across all four phases (this module's own documented assumption), including while stopped -- material for a vertical/inclined axis with a gravity-holding component and a nonzero dwell_time.",
