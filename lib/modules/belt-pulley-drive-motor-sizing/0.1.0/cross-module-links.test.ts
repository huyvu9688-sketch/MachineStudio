...
// module's real manifest.ts ports, the same exhaustive-sweep pattern every
// Motor Sizing Tool module's own cross-module-links.test.ts already
// establishes.
//
...
// Ltd.'s own combined wire-belt/rack-and-pinion sizing method -- this
// module has no compute-level dependency on any other released module.
// This module DOES reuse several already-released `motion.axis.*`
// parameter IDs directly (orientation, incline_angle, gravity,
// friction_coefficient, total_moving_mass) -- the same interface
// `ball-screw-motor-sizing@0.1.0` and `rack-pinion-motor-sizing@0.1.0`
// already reuse, so a real, exhaustive sweep is run below rather than a
// blanket "no links" claim.
...
// reuses -- the same real, incidental compatible pair
// ball-screw-motor-sizing@0.1.0's and rack-pinion-motor-sizing@0.1.0's own
// sweeps already found and documented for themselves. Excluded from the
// exhaustive "confirmed absent" sweep below so that sweep stays a genuine,
// currently-passing exhaustive check.
const KNOWN_COMPATIBLE_PAIRS: ReadonlySet<string> = new Set([
  "axis-load-cases.total_moving_mass->total_moving_mass",
]);

describe("belt-pulley-drive-motor-sizing 0.1.0 cross-module links: exhaustively confirmed absent, except one documented pair", () => {
  for (const upstream of UPSTREAM_MODULES) {
    it(`no ${upstream.label} 0.1.0 output is link-compatible with any belt-pulley-drive-motor-sizing input, other than the one documented exception`, () => {
      const sources = upstream.outputs.map((port) =>
        outputNode(port, `${upstream.label}-1`),
      );
      const sinks = ports.inputs.map((port) =>
        inputNode(port, "belt-pulley-drive-motor-sizing-1"),
      );
      ...
      for (const sink of sinks) {
        for (const source of sources) {
          ...
          const inputKey = sink.id.slice(
            "belt-pulley-drive-motor-sizing-1.".length,
          );
  ...
  it("confirms the one documented exception really is compatible: axis-load-cases' own resolved total_moving_mass output feeds this module's total_moving_mass input", () => {
    ...
    const sink = inputNode(
      ...
      "belt-pulley-drive-motor-sizing-1",
    );
    const result = evaluateLinkCompatibility(source, sink);
...
describe("belt-pulley-drive-motor-sizing 0.1.0 workflow role: deliberately none", () => {
