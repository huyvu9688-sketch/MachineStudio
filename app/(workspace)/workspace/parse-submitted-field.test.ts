...
describe("parseSubmittedField", () => {
  ...
  it("parses a boolean field, accepting any valueKind=boolean submission regardless of the parameter's declared valueType (matches setModuleInputValueAction's current lack of a valueType guard)", () => {
    const result = parseSubmittedField(
      buildFormData({
        "fields.enabled.parameterId": "motion.axis.payload_mass",
  ...
  it("returns an error for an unsupported value kind", () => {
    const result = parseSubmittedField(
      buildFormData({
        "fields.x.parameterId": "motion.axis.payload_mass",
        "fields.x.valueKind": "bogus",
      }),
      "x",
    );
    expect(result).toEqual({
      ok: false,
      message: 'Unsupported value kind "bogus".',
    });
  });
});
